    ...
    <div className="h-full flex gap-4 p-6 overflow-hidden">
      <section className="flex-1 flex flex-col min-w-0">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-xl font-bold text-slate-900">Weekly meals</h2>
          <div className="flex gap-2">
            <button onClick={() => setWeekOffset(w => w - 1)}
              className="px-4 py-2 bg-white border border-slate-200 hover:bg-slate-100 rounded-lg tap">←</button>
            <button onClick={() => setWeekOffset(0)}
              className="px-4 py-2 bg-white border border-slate-200 hover:bg-slate-100 rounded-lg tap text-sm">This week</button>
            <button onClick={() => setWeekOffset(w => w + 1)}
              className="px-4 py-2 bg-white border border-slate-200 hover:bg-slate-100 rounded-lg tap">→</button>
          </div>
        </div>
        <div className="flex-1 grid grid-cols-8 gap-1 overflow-auto bg-white border border-slate-200 rounded-2xl p-2">
          ...
          {days.map(d => (
            <div key={d.toISOString()} className="text-center py-2">
              ...
              <div className="text-base font-bold text-slate-900">{d.getDate()}</div>
            </div>
          ))}
          {MEAL_TYPES.map(type => (
              ...
              {days.map(d => {
                ...
                return (
                  <button
                    ...
                    className="min-h-[88px] rounded-lg p-2 text-sm bg-slate-50 hover:bg-slate-100 border border-slate-200 tap text-left"
                  >
                    <div className="text-slate-800 line-clamp-3">
                      {entry?.description || <span className="text-slate-400">—</span>}
                    </div>
                  </button>
                );
              })}
      ...
      <aside className="w-[360px] flex flex-col bg-white border border-slate-200 rounded-2xl overflow-hidden">
        <div className="p-4 border-b border-slate-200">
          <h2 className="text-xl font-bold mb-3 text-slate-900">Shopping list</h2>
          <div className="flex gap-2">
            <input
              value={newItem}
            ...
            <button onClick={addItem}
              className="px-4 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-semibold tap">+</button>
          </div>
        </div>
        <div className="flex-1 overflow-auto p-2">
          {shopping.length === 0 && (
            <div className="text-slate-400 text-center py-8 text-sm">List is empty</div>
          )}
          {shopping.map(it => (
            <div key={it.id} className="flex items-center gap-2 p-2 rounded-lg hover:bg-slate-50">
              ...
              <div className={`flex-1 text-sm ${it.checked ? 'text-slate-400 line-through' : 'text-slate-800'}`}>{it.item}</div>
              <button onClick={() => deleteItem(it.id)}
                className="w-9 h-9 rounded-md hover:bg-slate-100 text-slate-400 tap">×</button>
            </div>
          ))}
        </div>
        {shopping.some(s => s.checked) && (
          <div className="p-3 border-t border-slate-200">
            <button onClick={clearDone}
              className="w-full py-2 text-sm bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-lg tap text-slate-700">
              Clear checked
            </button>
          </div>
        )}
...
function MealEditModal({ editing, onSave, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm">
      <div className="bg-white border border-slate-200 rounded-2xl p-6 w-[480px] shadow-xl">
        <h3 className="text-lg font-bold mb-1 capitalize text-slate-900">{editing.type}</h3>
        <p className="text-sm text-slate-500 mb-4">{editing.date}</p>
        <textarea
          value={val}
          onChange={e => setVal(e.target.value)}
          placeholder="What's on the menu?"
          className="w-full min-h-[120px] bg-white border border-slate-200 rounded-lg px-3 py-2 text-slate-900"
        />
